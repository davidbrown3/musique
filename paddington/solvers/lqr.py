...
class LQR:
    ...
    def backward_pass(self, V_Ty, v_Ty):
        ...
        q_Tx = self.c_Tx + \
            np.dot(self.F_Tx.T, np.dot(V_Ty, self.f_Tx)) + \
            np.dot(self.F_Tx.T, v_Ty)

        # Q Value constant slices
        Q_xx_Tx = Q_Tx[0:self.N_x, 0:self.N_x]
        Q_uu_Tx = Q_Tx[self.N_x:(self.N_x+self.N_u),
                       self.N_x:(self.N_x+self.N_u)]
        ...
        V_Tx = Q_xx_Tx + np.dot(Q_xu_Tx, K_Tx) + np.dot(K_Tx.T,
                                                        Q_ux_Tx) + np.dot(K_Tx.T, np.dot(Q_uu_Tx, K_Tx))
        v_Tx = np.dot(Q_xu_Tx, k_Tx) + np.dot(K_Tx.T,
                                              np.dot(Q_uu_Tx, k_Tx)) + np.dot(K_Tx.T, q_u_Tx) + q_x_Tx
